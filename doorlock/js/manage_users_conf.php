    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
            ...
            if ($row['username'] != "Name"){
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)){
                        ...
                        $sql = "INSERT INTO users_logs (username, no_hp, fingerprint_id, checkindate, timein, timeout) VALUES (? ,?, ?, CURDATE(), CURTIME(), ?)";
                        $result = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        //New Fingerprint has been added
            ...
            if (!mysqli_stmt_prepare($result, $sql)) {
                ...
                mysqli_stmt_execute($result);
                $sql = "INSERT INTO users ( username, no_hp, gender, email, password, fingerprint_id, fingerprint_select, user_date, add_fingerid) VALUES (?, ?, ?, ?, ?, ?, 1, CURDATE(), 0)";
                $result = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($result, $sql)) {
