      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['username']=="Name") {
                  $cek_pass = mysqli_num_rows(mysqli_query($conn, "SELECT *FROM users WHERE password='$Pass'"));
            if ($cek_pass>0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE users SET username=?, phone_number=?, gender=?, email=?, password=?, user_date=CURDATE() WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                            ...
                            echo "The serial number is already taken!" .$Uname .$Number .$Gender .$Email .$Pass;
                            exit();
                        }
                    }
                }
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                              ...
                              exit();
                                ...
                                mysqli_stmt_execute($result);
                               $sql = "INSERT INTO users ( username, phone_number, gender, email, password, fingerprint_id, fingerprint_select, user_date, del_fingerid , add_fingerid) VALUES (?, ?, ?, ?, ?,?, 1, CURDATE(), 0, 1)";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                  echo "SQL_Error";
                                  exit();
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email) ) {
                                ...
                                $sql="UPDATE users SET username=?, phone_number=?, gender=?, email=?, password=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
