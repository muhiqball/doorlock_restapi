      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['username']=="Name") {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)&& !empty($Nohp)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, phone_number=?, user_date=CURDATE() WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                                ...
                                mysqli_stmt_bind_param($result, "sdssi", $Uname, $Number, $Gender, $Email, $Nohp);
                                mysqli_stmt_execute($result);
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                              ...
                              exit();
                                ...
                                mysqli_stmt_execute($result);
                                $sql = "INSERT INTO users ( username, serialnumber, gender, email, phone_number, fingerprint_id, fingerprint_select, user_date, del_fingerid , add_fingerid) VALUES (?, ?, ?, ?, ?, ?, 1, CURDATE() , 0, 1)";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                  echo "SQL_Error";
                                  exit();
                                    ...
                                    mysqli_stmt_bind_param($result, "sdssii", $Uname, $Number, $Gender, $Email, $Nohp, $fingerid);
                                    mysqli_stmt_execute($result);
                ...
                if (empty($Uname) && empty($Number) && empty($Email)&& empty($Nohp)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)&& !empty($Nohp)) {

                                $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, phone_number=?  WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
                                ...
                                if (!empty($Nohp)) {
                                    $sql="UPDATE users SET gender=?, phone_number=? WHERE fingerprint_select=1";
                                    $result = mysqli_stmt_init($conn);
                                    if (!mysqli_stmt_prepare($result, $sql)) {
                                        echo "SQL_Error_select_Fingerprint";
                                        exit();
                                        ...
                                        mysqli_stmt_bind_param($result, "si", $Gender, $Nohp);
                                        mysqli_stmt_execute($result);
