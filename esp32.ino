...
//************************************************************************
String getData, Link;
String URL = "http://192.168.43.36/doorlock/getdata.php"; //computer IP or the server domain
//************************************************************************
...
void loop() {
 // timer.run();      //Keep the timer in the loop function in order to update the time as soon as possible
  ...
  display.display();
    ...
    getData = "?pass=" + (temppass)+"&id"+String(id); // Add the Fingerprint ID to the Post array in order to send it
    String pasrol;
    //GET methode
    Link = URL + getData;
...
idtemp = payload.substring(6);
    Serial.println(httpCode);   //Print HTTP return code
    Serial.println(idtemp);
    Serial.println(payload);    //Print request response payload
     Serial.println(payload.substring(6));
     if (payload.substring(6) !="1"){
      if (temppass==payload.substring(0,6)){
 ...
 break;
  ...
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  display.println("Check your Web");
  display.println("Ready to Adding or");
  display.println("DELETING");
  display.setCursor(0, 54);
  display.println("to Adding or Delete.");
  display.display();
...
void SendFingerprintID( int finger ){
  ...
  if(WiFi.isConnected()){
    ...
    //GET Data
  String user;
    getData = "?user=" + (user); // Add the Fingerprint ID to the Post array in order to send it
